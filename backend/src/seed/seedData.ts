...
async function seed() {
  ...
  for (const def of THREAD_DEFINITIONS) {
    ...
    logger.info(`Seeding thread: "${def.title}" with ${def.messageCount} messages...`);
  ...
  const totalMessages = THREAD_DEFINITIONS.reduce((sum, d) => sum + d.messageCount, 0);
  logger.info(`Seeding complete: ${THREAD_DEFINITIONS.length} threads, ${totalMessages} messages in ${elapsed}s`);
